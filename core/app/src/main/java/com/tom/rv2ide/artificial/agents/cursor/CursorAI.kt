...
package com.tom.rv2ide.artificial.agents.cursor
...
class CursorAI : AIAgent {
  ...
  private var selectedModel: String = "cursor-small"
  override val providerId = "cursor"
  override val providerName = "Cursor AI"

  companion object {
      fun registerAgent() {
          AIAgentRegistry.register("cursor", object : AIAgentRegistry.AgentFactory {
              override fun create(context: Context): AIAgent {
                  return CursorAI()
              }
              
              override fun hasValidApiKey(): Boolean {
                  val key = ApiKey.getCursorApiKey()
                  android.util.Log.d("CursorAI", "hasValidApiKey check: ${key != null && key.isNotEmpty()}, key length: ${key?.length ?: 0}")
                  return key != null && key.isNotEmpty()
              }
              
              override fun getApiKey(): String? {
                  val key = ApiKey.getCursorApiKey()
                  android.util.Log.d("CursorAI", "getApiKey called, returning key of length: ${key?.length ?: 0}")
                  return key
              }
          })
      }
  ...
  override fun initialize(apiKey: String, context: Context) {
      try {
          ...
          var selectedModel = agents?.getAgent() ?: "cursor-small"
          
          // Ensure we're using a valid Cursor AI model
          if (!agents!!.isValidModelForProvider(selectedModel, "cursor")) {
              selectedModel = "cursor-small"
              agents?.setAgent(selectedModel)
              agents?.setProvider("cursor")
          }
        ...
        try {
          val key = apiKey
              ?: return@withContext Result.failure(
                  IllegalStateException("Cursor AI service not initialized")
              )
          ...
          val response = callCursorAPI(key, fullPrompt)
  ...
  private fun callCursorAPI(apiKey: String, prompt: String): String {
    android.util.Log.d("CursorAI", "Starting API call to Cursor AI")
    
    val url = URL("https://api.cursor.sh/v1/chat/completions")
    ...
    try {
      ...
      android.util.Log.d("CursorAI", "Request body: ${requestBody.toString()}")
      ...
      android.util.Log.d("CursorAI", "Response code: $responseCode")
      
      if (responseCode != HttpURLConnection.HTTP_OK) {
        val errorStream = connection.errorStream?.bufferedReader()?.readText() ?: "Unknown error"
        android.util.Log.e("CursorAI", "Error response: $errorStream")
        ...
        try {
          ...
          android.util.Log.e("CursorAI", "Error type: $errorType, code: $errorCode, message: $errorMessage")
          
          when {
            responseCode == 429 || errorType.contains("rate_limit") || errorCode.contains("rate_limit") -> 
              throw com.tom.rv2ide.artificial.exceptions.RateLimitException("Cursor AI rate limit exceeded: $errorMessage")
            errorType.contains("insufficient_quota") || errorMessage.contains("quota") || errorMessage.contains("billing") -> 
              throw com.tom.rv2ide.artificial.exceptions.QuotaExceededException("Cursor AI quota exceeded: $errorMessage")
            errorType.contains("invalid_api_key") || errorCode.contains("invalid_api_key") -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("Invalid Cursor AI API key: $errorMessage")
            responseCode == 401 -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("Cursor AI authentication failed: $errorMessage")
            else -> 
              throw Exception("Cursor AI API error ($responseCode) - Type: $errorType, Code: $errorCode, Message: $errorMessage")
          }
          ...
          throw Exception("Cursor AI API error ($responseCode): $errorStream")
        }
      }
      
      val responseBody = connection.inputStream.bufferedReader().readText()
      android.util.Log.d("CursorAI", "Success response received, length: ${responseBody.length}")
      ...
      throw Exception("No response from Cursor AI API")
    } catch (e: com.tom.rv2ide.artificial.exceptions.RateLimitException) {
      android.util.Log.e("CursorAI", "Rate limit exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.QuotaExceededException) {
      android.util.Log.e("CursorAI", "Quota exceeded exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException) {
      android.util.Log.e("CursorAI", "Invalid API key exception", e)
      throw e
    } catch (e: java.net.SocketTimeoutException) {
      android.util.Log.e("CursorAI", "Timeout exception", e)
      throw Exception("Cursor AI request timeout: ${e.message}")
    } catch (e: java.net.UnknownHostException) {
      android.util.Log.e("CursorAI", "Network exception", e)
      throw Exception("Network error - cannot reach Cursor AI: ${e.message}")
    } catch (e: Exception) {
      android.util.Log.e("CursorAI", "General exception", e)
      throw e
    } finally {
