...
package com.tom.rv2ide.artificial.agents.letta
...
class LettaAI : AIAgent {
  ...
  private var selectedModel: String = "letta-free"
  override val providerId = "letta"
  override val providerName = "Letta AI"

  companion object {
      fun registerAgent() {
          AIAgentRegistry.register("letta", object : AIAgentRegistry.AgentFactory {
              override fun create(context: Context): AIAgent {
                  return LettaAI()
              }
              
              override fun hasValidApiKey(): Boolean {
                  val key = ApiKey.getLettaApiKey()
                  android.util.Log.d("LettaAI", "hasValidApiKey check: ${key != null && key.isNotEmpty()}, key length: ${key?.length ?: 0}")
                  return key != null && key.isNotEmpty()
              }
              
              override fun getApiKey(): String? {
                  val key = ApiKey.getLettaApiKey()
                  android.util.Log.d("LettaAI", "getApiKey called, returning key of length: ${key?.length ?: 0}")
                  return key
              }
          })
      }
  ...
  override fun initialize(apiKey: String, context: Context) {
      try {
          ...
          var selectedModel = agents?.getAgent() ?: "letta-free"
          
          // Ensure we're using a valid Letta AI model
          if (!agents!!.isValidModelForProvider(selectedModel, "letta")) {
              selectedModel = "letta-free"
              agents?.setAgent(selectedModel)
              agents?.setProvider("letta")
          }
        ...
        try {
          val key = apiKey
              ?: return@withContext Result.failure(
                  IllegalStateException("Letta AI service not initialized")
              )
          ...
          val response = callLettaAPI(key, fullPrompt)
  ...
  private fun callLettaAPI(apiKey: String, prompt: String): String {
    android.util.Log.d("LettaAI", "Starting API call to Letta AI")
    
    val url = URL("https://api.letta.com/v1/chat/completions")
    ...
    try {
      ...
      android.util.Log.d("LettaAI", "Request body: ${requestBody.toString()}")
      ...
      android.util.Log.d("LettaAI", "Response code: $responseCode")
      
      if (responseCode != HttpURLConnection.HTTP_OK) {
        val errorStream = connection.errorStream?.bufferedReader()?.readText() ?: "Unknown error"
        android.util.Log.e("LettaAI", "Error response: $errorStream")
        ...
        try {
          ...
          android.util.Log.e("LettaAI", "Error type: $errorType, code: $errorCode, message: $errorMessage")
          
          when {
            responseCode == 429 || errorType.contains("rate_limit") || errorCode.contains("rate_limit") -> 
              throw com.tom.rv2ide.artificial.exceptions.RateLimitException("Letta AI rate limit exceeded: $errorMessage")
            errorType.contains("insufficient_quota") || errorMessage.contains("quota") || errorMessage.contains("billing") -> 
              throw com.tom.rv2ide.artificial.exceptions.QuotaExceededException("Letta AI quota exceeded: $errorMessage")
            errorType.contains("invalid_api_key") || errorCode.contains("invalid_api_key") -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("Invalid Letta AI API key: $errorMessage")
            responseCode == 401 -> 
              throw com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException("Letta AI authentication failed: $errorMessage")
            else -> 
              throw Exception("Letta AI API error ($responseCode) - Type: $errorType, Code: $errorCode, Message: $errorMessage")
          }
          ...
          throw Exception("Letta AI API error ($responseCode): $errorStream")
        }
      }
      
      val responseBody = connection.inputStream.bufferedReader().readText()
      android.util.Log.d("LettaAI", "Success response received, length: ${responseBody.length}")
      ...
      throw Exception("No response from Letta AI API")
    } catch (e: com.tom.rv2ide.artificial.exceptions.RateLimitException) {
      android.util.Log.e("LettaAI", "Rate limit exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.QuotaExceededException) {
      android.util.Log.e("LettaAI", "Quota exceeded exception", e)
      throw e
    } catch (e: com.tom.rv2ide.artificial.exceptions.InvalidApiKeyException) {
      android.util.Log.e("LettaAI", "Invalid API key exception", e)
      throw e
    } catch (e: java.net.SocketTimeoutException) {
      android.util.Log.e("LettaAI", "Timeout exception", e)
      throw Exception("Letta AI request timeout: ${e.message}")
    } catch (e: java.net.UnknownHostException) {
      android.util.Log.e("LettaAI", "Network exception", e)
      throw Exception("Network error - cannot reach Letta AI: ${e.message}")
    } catch (e: Exception) {
      android.util.Log.e("LettaAI", "General exception", e)
      throw e
    } finally {
